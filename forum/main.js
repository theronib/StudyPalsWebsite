...
success: function(data) {
    ...
    for (var i=0; i<data.length; i++) {
        ...
        var row = $('<tr><td><b style = "color: #0b5967;">@' + data[i].student + ' ['+ data[i].date + ']</i></b></br><p style="color: #0b5967;">' + data[i].post + '</br><a data-toggle="modal" data-id="'+ commentId +'" title="Add this item" class="open-ReplyModal" href="#ReplyModal">Відповісти</a>'+'</p></td></tr>');
        $('#record').append(row);
        for (var r = 0; (r < data.length); r++)
                {
                    if ( data[r].parent_comment == commentId)
                    {
                        var comments = $('<tr><td style="padding-left:80px"><b style = "color: #0b5967;">@' + data[r].student + ' [' + data[r].date + ']</i></b></br><p style="color: #0b5967;">'+ data[r].post +'</p></td></tr>');
